...
<div class="col-lg-12">
    <div class="card" id="customerList">
        ...
        <form action="{{ route('admin.danh_mucs.store') }}" autocomplete="off" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
                <div class="body row">
                    <div class="mb-3 col-12">
                        <label for="ten_danh_muc" class="form-label">Tên danh mục</label>
                        <input value="{{ old('ten_danh_muc') }}" name="ten_danh_muc" type="text" id="ten_danh_muc" class="form-control" placeholder="Enter name">
                        @error('ten_danh_muc')
                    ...
                    <div class="mb-3 col-6">
                        ...
                        <input type="hidden" name="old_hinh_anh" value="{{ old('hinh_anh', $danhMuc->hinh_anh ?? '') }}">

                        <input name="hinh_anh" type="file" id="hinh_anh" class="form-control">
                        @php
                        $oldImagePath = session('hinh_anh_cu');
                        @endphp

                        @if($oldImagePath)
                        <img src="{{ asset('storage/' . $oldImagePath) }}" alt="Old Image" width="100">
                        @endif
                        @error('hinh_anh')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
