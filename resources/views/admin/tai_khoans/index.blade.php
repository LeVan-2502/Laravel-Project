    ...
    <div class="page-title-right">
        <ol class="breadcrumb m-0">
            <li class="breadcrumb-item">
                <a href="{{ route('admin.tai_khoans.' . $rou) }}">{{ $table }}</a>
            </li>

            <li class="breadcrumb-item active">Danh sách {{$title}}</li>
        </ol>
    </div>
    ...
    <div class="card" id="customerList">
        <div class="card-header border-bottom-dashed">
            ...
            <div class="row g-4 align-items-center">
                ...
                <div class="col-sm">
                    <div>
                        <h5 class="card-title mb-0">Danh sách {{$title}}</h5>
                    </div>
                </div>
                <div class="col-sm-auto">
                    <div class="d-flex flex-wrap align-items-start gap-2">
                        <a href="{{ route('admin.tai_khoans.create')}}" class="btn btn-success add-btn"><i class="ri-add-line align-bottom me-1"></i>Thêm {{$table}}</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <form id="deleteForm" action="{{ route('admin.tai_khoans.delete-selected') }}" method="POST">
            @method('DELETE')
            @csrf
            <div class="card-body">
                <!-- Success Alert -->
                        ...
                        <table class="table align-middle" id="danhmucTable">
                            <thead class="table-light text-muted">
                                <tr>
                                    ...
                                    <th class="sort" data-sort="ten_tai_khoan">Tên tài khoản</th>
                                    <th class="sort" data-sort="hinh_anh">Hình ảnh</th>
                                    <th class="sort" data-sort="email">Email</th>
                                    <th class="sort" data-sort="gioi_tinh">Giới tính</th>
                                    <th class="sort" data-sort="trang_thai">Trạng thái</th>
                            ...
                            <tbody class="list form-check-all">
                                ...
                                <tr>
                                    ...
                                    <td class="ten_tai_khoan">{{ $item->ten_tai_khoan }}</td>
                                    <td class="hinh_anh">
                                        @if (!empty($item->hinh_anh))
                                        <img src="{{ \Storage::url($item->hinh_anh) }}" width="50px" alt="Hình ảnh danh mục">
                                        @else
                                        <img src="{{ asset('he_thongs/avatar_default.jpeg') }}" width="50px" alt="Hình ảnh danh mục">
                                    ...
                                    <td class="gioi_tinh">
                                        @if ($item->gioi_tinh == 1)
                                        <span class="badge rounded-pill bg-warning">Nam</span>
                                        @elseif($item->gioi_tinh == 2)
                                        <span class="badge rounded-pill bg-info">Nữ</span>
                                        @else 
                                        <span class="badge rounded-pill bg-secondary">Khác</span>
                                    ...
                                    <td>
                                        <ul class="list-inline hstack gap-2 mb-0">
                                            ...
                                            <li class="list-inline-item edit" title="Edit">
                                                <a href="{{ route('admin.tai_khoans.edit', $item->id)}}" class="text-primary d-inline-block edit-item-btn">
                                                    <i class="ri-pencil-fill fs-16"></i>
                                                </a>
                                            </li>
                                            <!-- Remove Button -->
                                            <li class="list-inline-item" title="Remove">
                                                <a onclick="return confirm('Bạn đã chắc chắn chưa?')" href="{{ route('admin.tai_khoans.destroy', $item->id) }}">
                                                    <i style="color:red;" class="ri-delete-bin-5-fill fs-16"></i>
                                                </a>
                                            </li>
                                            <li>
                                                <a href="{{ route('admin.tai_khoans.show', $item->id)}}" class="text-primary d-inline-block edit-item-btn">
